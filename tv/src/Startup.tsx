 ...
 return <MotionContext.Provider value={reduced}><FocusScope.Provider value="intro-story"><View style={s.root} testID="premiere-intro">
  ...
  <View style={[s.footer,{paddingHorizontal:small?24:56}]}>
   <View testID="intro-actions" style={[s.row,{justifyContent:'space-between'}]}>
    <View style={s.row}><TVButton primary preferred onPress={finish}>Skip intro →</TVButton><TVButton onPress={()=>{if(reduced||paused){if(step===eveningStory.length-1)finish();else setStep(step+1);}else setPaused(true);}}>{reduced||paused?step===eveningStory.length-1?'Start my evening →':'Next scene →':'Pause story'}</TVButton>{paused&&!reduced&&<TVButton quiet onPress={()=>setPaused(false)}>Resume story</TVButton>}{(paused||reduced)&&step>0&&<TVButton quiet onPress={()=>setStep(step-1)}>Previous</TVButton>}</View>
