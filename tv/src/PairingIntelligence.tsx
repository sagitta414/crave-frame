...
export function connectionType(pair:any){
 if(pair?.pairingType)return pair.pairingType==='story'?{label:'FOOD FROM THE STORY',color:'#F4CE65',description:'Documented food; our recipe interpretation'}:pair.pairingType==='setting'?{label:'STORY-INSPIRED DINNER',color:'#73C9F4',description:'Creative interpretation, not verified on-screen food'}:{label:'JUST A GOOD DINNER',color:'#78DBB5',description:'Practical choice without a strong story connection'};
...
export function DecisionReceipt({receipt}:any){
 ...
 return <View style={i.panel}><View style={i.rowBetween}><View><Text style={i.eyebrow}>HOW WE CHOSE THESE</Text><Text style={i.title}>Your preferences shaped these choices</Text></View><Button quiet onPress={()=>setOpen(!open)}>{open?'Hide details':'See details'}</Button></View>{open&&<><View style={i.funnel}>{[
  [receipt.eligibleMeals,'meals that fit'],[receipt.eligibleShows,'matching titles'],[receipt.candidatePairings,'combinations considered'],[receipt.finalists,'finalists'],[1,'recommended']
 ].map((x:any,n)=><View key={x[1]} style={[i.funnelStep,{width:`${100-n*10}%`}]}><Text style={i.funnelNumber}>{x[0]}</Text><Text style={i.funnelText}>{x[1]}</Text></View>)}</View>{receipt.constraints?.length>0&&<Text style={i.copy}>Applied: {receipt.constraints.join(' · ')}</Text>}{receipt.alternatives?.map((x:any)=><View key={x.name} style={i.alternative}><Text style={i.eyebrow}>ANOTHER OPTION</Text><Text style={i.altName}>{x.name}</Text><Text style={i.copy}>{x.whyNotFirst}</Text></View>)}<Text style={i.note}>Counts come from the eligible catalog used for this request. AI ranks the finalists; catalog rules enforce the limits.</Text></>}</View>;
}
