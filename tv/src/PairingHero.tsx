...
export function PairingHero({pair,source,pending=false,watchArt,onChoose,onDetails,onSearch,position=0,count=1,onPosition}:any){
 ...
 if(matched)return <View testID="remote-row-spotlight" style={{backgroundColor:'#070B14',paddingHorizontal:wide?48:24,paddingVertical:24,gap:20}}><Text style={s.label}>TONIGHT’S PICK</Text><View style={{flexDirection:wide?'row':'column',gap:24,alignItems:'stretch'}}><View style={{flex:1,minWidth:0}}><Image accessible={false} source={watchArt(pair.show,true)} resizeMode="cover" style={{width:'100%',height:wide?Math.min(380,Math.max(260,width*.22)):220,borderRadius:18}}/><Text accessibilityRole="header" style={[s.title,{fontSize:wide?44:34,lineHeight:wide?52:42,marginTop:16,marginBottom:8}]}>{pair.show.name}</Text><Text style={s.meta}>{pair.show.kind} · {pair.show.minutes} min watching</Text></View><View style={{flex:1,minWidth:0}}><RecipeArt meal={pair.meal} style={{width:'100%',height:wide?Math.min(380,Math.max(260,width*.22)):240,borderRadius:18}}/><Text style={[s.meal,{fontSize:wide?36:30,lineHeight:wide?46:40,marginTop:16}]}>{pair.meal.name}</Text><Text style={s.meta}>{pair.meal.minutes} min cooking</Text></View></View><View style={{flexDirection:'row',flexWrap:'wrap',alignItems:'center',gap:12}}><Button compact primary preferred focusKey="hero-choose" onPress={()=>onChoose(pair)}>Plan this evening</Button><Button compact quiet focusKey="hero-search" onPress={onSearch}>Change title</Button>{count>1&&Array.from({length:count},(_,i)=><Button compact key={i} quiet selected={position===i} focusKey={'spotlight-'+i} onPress={()=>onPosition(i)}>{'0'+(i+1)}</Button>)}</View>{pending&&<Text style={s.note}>Updating your dinner choices…</Text>}</View>;
