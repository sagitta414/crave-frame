...
export function RecipeArt({meal,style}:any){const photo=mealPhotos[meal?.id], [failed,setFailed]=useState<any>(null);
 if(photo&&failed!==photo)return <View accessibilityLabel={`${meal?.name || 'Dish'}${generatedMealPhotos[meal?.id] ? ', AI-generated illustration' : ''}`} style={[{overflow:'hidden'},style]}><Image source={photo} resizeMode="cover" onError={()=>setFailed(photo)} style={{width:'100%',height:'100%'}}/>{generatedMealPhotos[meal?.id] && <View style={{position:'absolute',bottom:0,left:0,right:0,backgroundColor:'#101B2DEE',paddingVertical:4,paddingHorizontal:8,alignItems:'center'}}><Text style={{color:'#E8EDF5',fontSize:24,lineHeight:30}}>AI illustration</Text></View>}</View>;
