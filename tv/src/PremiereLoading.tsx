...
export function PremiereLoading({reduced=false,boot=false,onCancel}:{reduced?:boolean,boot?:boolean,onCancel?:()=>void}){
 ...
 const [step,setStep]=useState(0);useEffect(()=>{if(reduced||paused||!shown||step===2)return;const timer=setTimeout(()=>setStep(v=>Math.min(2,v+1)),5500);return()=>clearTimeout(timer);},[shown,step,reduced,paused]);
